...
def get_free_gpu():
    ...
    for gpu_id in visible_devices:
        try:
            # Query free memory for the specific GPU
    ...
    if len(free_memory) > 0:
        ...
        id = int(np.argmax(free_memory))
        global_gpu_id = visible_devices[id]  # Map back to global GPU ID
        print(f"Setting to GPU:{global_gpu_id}")
        torch.cuda.set_device(global_gpu_id)
        return f"cuda:{global_gpu_id}"
    else:
        # If no GPU has free memory, randomly select from the allocated GPUs
        id = int(np.random.choice(visible_devices, 1))
        print(f"Setting to GPU:{id}")
